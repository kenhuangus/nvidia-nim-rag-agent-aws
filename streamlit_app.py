    ...
    with st.expander("📚 View All Documents", expanded=False):
        ...
        if doc_count > 0:
            ...
            try:
                ...
                # Display by source (no nested expanders)
                for source, docs in docs_by_source.items():
                    st.markdown(f"### 📄 {source} ({len(docs)} chunks)")

                    # Show only first 5 chunks by default, with option to show more
                    display_count = min(5, len(docs))
                    show_all = st.checkbox(f"Show all {len(docs)} chunks", key=f"show_all_{source}")

                    if show_all:
                        display_count = len(docs)

                    for i, doc in enumerate(docs[:display_count]):
                        # Apply search filter
                    ...
                    if len(docs) > display_count:
                        st.caption(f"Showing {display_count} of {len(docs)} chunks. Check 'Show all' to see more.")
...
def main():
    ...
    # Sidebar for monitoring and settings
    with st.sidebar:
        # Document viewer
        ...
        with st.expander("⚙️ Settings"):
            ...
            st.caption(f"Model: {settings.nim_model}")
    ...
    # Chat input (must be outside tabs/expanders)
